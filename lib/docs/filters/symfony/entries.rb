module Docs
  class Symfony
    class EntriesFilter < Docs::EntriesFilter
      ...
      def get_type
        return "Exceptions" if /exception/i.match?(slug)
        return "Testing" if /test/i.match?(slug)
        namespace
      end
      ...
      def additional_entries
        ...
        return [] if name.include?("Legacy") || name.include?("Loader")

        entries = []
        base_name = name.remove(/\(.+\)/).strip
