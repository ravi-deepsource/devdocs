  ...
  class Gnuplot
    ...
    class EntriesFilter < Docs::EntriesFilter
      ...
      def include_default_entry?
        !root_page? && (slug.downcase != "complete_list_terminals") # and !PROMOTE.include? name
      end
      ...
      def additional_entries
        ...
        list_stack = if slug.downcase == "complete_list_terminals"
          [[css("ul.ChildLinks"), "", nil]]
        else
          [[css("ul.ChildLinks"), name, nil]]
        end
        ...
        until list_stack.empty?
          ...
          list.css("> li").each do |item|
            ...
            if link.empty?
              ...
              item_name = "#{name_} #{item_name}".strip unless PROMOTE.include?(name_) || NOREPEAT.include?(name_)
              item_name = item_name.sub(/^(\w+) \1/, '\1')
              item_name = "set style boxplot" if (slug.downcase == "set_show") && (item_name == "Boxplot")
