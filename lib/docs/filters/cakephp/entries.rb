    ...
    class EntriesFilter < Docs::EntriesFilter
      ...
      def get_type
        ...
        when "class", "namespace", "package"
          path = if (node = at_css(".info")) && node.content =~ /Location:\s+((?:\w+\/?)+)/ # for 2.x docs
            $1.split("/")
          else
            slug_without_page_type.split(".")
          end
      ...
      def additional_entries
        ...
        css("h3.method-name").each do |node|
          break if /\AMethods.*from/.match?(node.parent.previous_element.content)
